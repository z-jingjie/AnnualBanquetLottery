  ...
  <div class="goods-stock-manage">
    ...
    <div v-loading="goods_loading" class="goods-display-table">
      <el-row class="goods-table-head">
        ...
        <el-col :span="3">
          <span class="table-head-txt">ID</span>
        </el-col>
        <el-col :span="3">
      ...
      <el-row v-for="(goods, index) in goods_list" v-if="goods_list" :class="{active: goods.is_checked,edited:edit_list[goods.goods_id]}" :key="goods.goods_id" class="goods-table-item" type="flex">
        ...
        <el-col :span="3">
          <p class="table-item-txt goods-cat">{{ goods._id }}</p>
        </el-col>
      ...
      <p v-if="no_goods_find" class="no-table-content-warn">没有抽奖码记录</p>
    </div>
...
export default {
  ...
  methods: {
      ...
      deleteAll() {
          ...
          this.$confirm('确定要清空全部抽奖码吗', '警告', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
          }).then(() => {
              request({
                  url: '/lottery_code/del',
                  method: 'post',
              }).then((res) => {
                  if(res.data.status===1000) {
                      this.$message({
      ...
      submit() {
          this.$refs['cate_info'].validate((valid) => {
              if (valid) {
                  let url = this.is_add ? '/lottery_code/add' : '/lottery_code/edit';
    ...
    exportAllGoodsExcel() {
          ...
          location.href = '/2018annualparty/api/lottery_code/export_csv';

          return;
        request({
            url: '/lottery_code/export_csv',
            method: 'get',
        }).then((res) => {
            if(res.data.status===1000) {
                this.$message({
